function audit = audit_trained_models(modelDir, outDir)
%AUDIT_TRAINED_MODELS Summarize training metadata and validation histories.
%
% This is a post-training diagnostic stage used before closed-loop
% experiments. It checks whether the newly trained checkpoints are
% internally consistent and whether the proposed model is actually
% outperforming simpler baselines on validation/test summaries.
...
if nargin < 1; modelDir = fullfile(projectRoot, 'data', 'trained_models'); end
if nargin < 2; outDir = fullfile(projectRoot, 'data', 'eval_results', 'training_audit'); end
...
for i = 1:size(specs, 1)
    ...
    rows(i) = inspect_one(modelPath, key); %#ok<AGROW>
    audit.(key) = rows(i);
end
...
disp(T);
fprintf('[audit_trained_models] saved to %s\n', outDir);
end
