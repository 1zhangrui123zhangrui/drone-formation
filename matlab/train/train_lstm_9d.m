...
%TRAIN_LSTM_9D Train the 9D single-direction LSTM baseline.
...
if nargin < 2; modelPath = fullfile(projectRoot, 'data', 'trained_models', 'c1_lstm9d.mat'); end
...
assert(isnumeric(Y_tr) && isreal(Y_tr) && all(isfinite(Y_tr(:))), ...
    '[C1] Training responses must be finite real numbers.');
assert(isnumeric(Y_va) && isreal(Y_va) && all(isfinite(Y_va(:))), ...
    '[C1] Validation responses must be finite real numbers.');
...
trainFile = 'dataset_9d_train.mat';
valFile = 'dataset_9d_val.mat';
[net, info] = trainNetwork(X_tr, Y_tr, layers, opts);

metadata = build_training_metadata('c1_lstm9d', dataDir, trainFile, valFile, tr, va);
