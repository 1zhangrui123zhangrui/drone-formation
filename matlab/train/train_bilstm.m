...
%TRAIN_BILSTM Train the 15D BiLSTM model without attention.
...
if nargin < 2; modelPath = fullfile(projectRoot, 'data', 'trained_models', 'c3a_bilstm.mat'); end
...
assert(isnumeric(Y_tr) && isreal(Y_tr) && all(isfinite(Y_tr(:))), ...
    '[C3a] Training responses must be finite real numbers.');
assert(isnumeric(Y_va) && isreal(Y_va) && all(isfinite(Y_va(:))), ...
    '[C3a] Validation responses must be finite real numbers.');
...
metadata = build_training_metadata('c3a_bilstm', dataDir, trainFile, valFile, tr, va);
